...
const AdminDashboard: React.FC<AdminDashboardProps> = ({ 
  ...
  // Action Handlers
  const openConfirmation = (id: string | null, type: 'Confirm' | 'Cancel' | 'Complete' | 'Trash' | 'Restore' | 'BulkTrash' | 'BulkCancel' | 'BulkComplete' | 'ResetDatabase' | 'DeleteService' | 'BulkEnableServices' | 'BulkDisableServices' | 'BulkDeleteServices' | 'DeleteInquiry' | 'BulkDeleteInquiries' | 'BulkInquiryMarkRead') => {
  ...
  const handleConfirmAction = () => {
    if (confirmationAction.type) {
      ...
      if (confirmationAction.type === 'ResetDatabase') {
         ...
         else if (confirmationAction.type === 'BulkInquiryMarkRead') {
            onBulkInquiryMarkRead(ids);
  ...
  const inquiryTabs: (InquiryStatus | 'All')[] = ['All', 'New', 'Contacted', 'Follow-up Needed', 'Resolved'];
  ...
  return (
    <div className="min-h-screen bg-gray-100 flex animate-fade-in relative overflow-hidden">
      ...
      <main className="flex-1 md:ml-64 p-4 md:p-8 overflow-y-auto h-screen transition-all duration-300">
        ...
        {activeTab === 'appointments' && (
          <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden animate-fade-in flex flex-col h-[calc(100vh-140px)]">
                       ...
                       <div className="flex items-center gap-2 animate-fade-in relative">
                          ...
                          <div className="relative">
                             <button 
                                ...
                                className="bg-white border border-gray-200 text-gray-700 px-3 py-2 rounded-lg text-sm font-medium flex items-center gap-2 shadow-sm hover:bg-gray-50"
                             >
                                Bulk Actions <ChevronDown size={14} />
                             </button>
                             {isBulkDropdownOpen && (
                                <div className="absolute top-full left-0 mt-1 w-48 bg-white border border-gray-100 rounded-lg shadow-xl z-20 overflow-hidden animate-fade-in">
                                   <button onClick={() => { openConfirmation(null, 'BulkComplete'); setIsBulkDropdownOpen(false); }} className="w-full text-left px-4 py-2 text-sm text-blue-600 hover:bg-blue-50 flex items-center gap-2"><CheckSquare size={14}/> Mark Completed</button>
                                   <button onClick={() => { openConfirmation(null, 'BulkCancel'); setIsBulkDropdownOpen(false); }} className="w-full text-left px-4 py-2 text-sm text-red-600 hover:bg-red-50 flex items-center gap-2"><XCircle size={14}/> Cancel Selected</button>
                                   <button onClick={() => { openConfirmation(null, 'BulkTrash'); setIsBulkDropdownOpen(false); }} className="w-full text-left px-4 py-2 text-sm text-gray-600 hover:bg-gray-50 flex items-center gap-2 border-t border-gray-100"><Trash2 size={14}/> Move to Trash</button>
                                </div>
                             )}
                          </div>
                       </div>
             ...
             <div className="md:hidden flex-1 overflow-y-auto p-4 space-y-4">
               {paginatedAppointments.map(apt => (
                 <div key={apt.id} className={`bg-white p-4 rounded-xl border shadow-sm ${selectedIds.has(apt.id) ? 'border-gold-400 bg-gold-50' : 'border-gray-100'}`}>
                    <div className="flex justify-between items-start mb-3">
                       <div className="flex items-center gap-3">
                          <input type="checkbox" checked={selectedIds.has(apt.id)} onChange={() => handleSelectOne(apt.id)} className="w-5 h-5 rounded border-gray-300 text-gold-600" />
                          <div>
                             <div className="font-bold text-gray-900">{apt.clientName}</div>
                             <div className="text-xs text-gray-500">{apt.id}</div>
                          </div>
                       </div>
                       <span className={`px-2 py-1 rounded-full text-[10px] font-bold border ${getStatusBadge(apt.status)}`}>{apt.status}</span>
                    </div>
                    <div className="space-y-2 mb-4">
                       <div className="flex items-center gap-2 text-sm text-gray-700"><Briefcase size={14} className="text-gold-500"/> {apt.service}</div>
                       <div className="flex items-center gap-2 text-sm text-gray-600"><Clock size={14} className="text-gold-500"/> {new Date(apt.date).toLocaleDateString()} @ {apt.time}</div>
                       <div className="flex items-center gap-2 text-sm text-gray-600"><Phone size={14} className="text-gold-500"/> {apt.phone}</div>
                    </div>
                    <div className="flex justify-between items-center pt-3 border-t border-gray-100">
                       <button onClick={() => setViewAppointment(apt)} className="text-xs font-bold text-gray-500 hover:text-gold-600">View Details</button>
                       <div className="flex gap-2">
                          {apt.status === 'Pending' && <button onClick={() => openConfirmation(apt.id, 'Confirm')} className="p-1.5 bg-green-100 text-green-600 rounded"><CheckCircle size={16}/></button>}
                          {apt.status === 'Confirmed' && <button onClick={() => openConfirmation(apt.id, 'Complete')} className="p-1.5 bg-blue-100 text-blue-600 rounded"><CheckSquare size={16}/></button>}
                          {(apt.status === 'Confirmed' || apt.status === 'Pending') && <button onClick={() => openConfirmation(apt.id, 'Cancel')} className="p-1.5 bg-red-100 text-red-600 rounded"><XCircle size={16}/></button>}
                          {(apt.status === 'Completed' || apt.status === 'Cancelled') && <button onClick={() => openConfirmation(apt.id, 'Trash')} className="p-1.5 bg-gray-100 text-gray-500 rounded"><Trash2 size={16}/></button>}
                       </div>
                    </div>
                 </div>
               ))}
             ...
             <div className="p-4 border-t border-gray-100 flex items-center justify-between bg-gray-50 flex-shrink-0">
               ...
               <div className="flex gap-2">
                  <button onClick={() => setCurrentPage(prev => Math.max(prev - 1, 1))} disabled={currentPage === 1} className="p-2 border border-gray-300 rounded hover:bg-gray-100 disabled:opacity-50 disabled:hover:bg-transparent"><ChevronLeft size={16} /></button>
                  <button onClick={() => setCurrentPage(prev => Math.min(prev + 1, totalPages))} disabled={currentPage === totalPages || totalPages === 0} className="p-2 border border-gray-300 rounded hover:bg-gray-100 disabled:opacity-50 disabled:hover:bg-transparent"><ChevronRight size={16} /></button>
               </div>
             </div>
          </div>
        )}
        ...
        {/* CLIENTS TAB */}
        {activeTab === 'clients' && (
           <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden animate-fade-in pb-10 flex flex-col h-[calc(100vh-140px)]">
              <div className="p-4 md:p-6 border-b border-gray-100 flex flex-col md:flex-row justify-between items-start md:items-center gap-4 flex-shrink-0">
                 <h2 className="text-lg font-bold text-gray-900">Client Directory</h2>
                 <div className="relative w-full md:w-64">
                   <Search className="absolute left-3 top-2.5 text-gray-400" size={16} />
                   <input type="text" placeholder="Search clients..." className="w-full pl-9 pr-4 py-2 text-sm border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-gold-500" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} />
                 </div>
              </div>
                      ...
                      <tbody className="divide-y divide-gray-100 text-sm">
                         {uniqueClients.map((client, idx) => (
                            <tr key={idx} className="hover:bg-gray-50 transition-colors">
                               <td className="p-4"><div className="font-bold text-gray-900">{client.name}</div></td>
                               <td className="p-4"><div className="flex items-center gap-2 text-gray-600 mb-1"><Phone size={14} /> {client.phone}</div><div className="flex items-center gap-2 text-gray-600"><Mail size={14} /> {client.email}</div></td>
                               <td className="p-4 text-center"><span className="bg-gray-100 px-2 py-1 rounded text-gray-700 font-bold">{client.totalVisits}</span></td>
                               <td className="p-4 text-gray-600">{client.lastVisit}</td>
                               <td className="p-4"><span className={`px-2 py-1 rounded-full text-xs font-bold border ${client.status === 'Active' ? 'bg-green-50 text-green-700 border-green-200' : 'bg-gray-100 text-gray-500 border-gray-200'}`}>{client.status}</span></td>
                               <td className="p-4 text-right"><button onClick={() => setViewClientHistory(client.name)} className="text-gold-600 hover:text-gold-800 font-bold text-xs flex items-center gap-1 justify-end"><History size={14} /> View History</button></td>
                            </tr>
                         ))}
                         {uniqueClients.length === 0 && (<tr><td colSpan={6} className="p-8 text-center text-gray-500">No clients found.</td></tr>)}
                      </tbody>
                   </table>
                 </div>
                    ...
                    {uniqueClients.map((client, idx) => (
                       <div key={idx} className="bg-white p-4 rounded-xl border border-gray-200 shadow-sm">
                          <div className="flex justify-between items-start mb-2">
                             ...
                             <span className={`px-2 py-1 rounded-full text-[10px] font-bold border ${client.status === 'Active' ? 'bg-green-50 text-green-700 border-green-200' : 'bg-gray-100 text-gray-500 border-gray-200'}`}>{client.status}</span>
                          </div>
                          <div className="text-sm text-gray-600 space-y-1 mb-3">
                             ...
                             <div className="flex items-center gap-2"><CalendarIcon size={14}/> Last Visit: {client.lastVisit}</div>
                          </div>
                          <div className="flex justify-between items-center pt-3 border-t border-gray-100">
                             <span className="text-xs font-bold bg-gray-100 px-2 py-1 rounded text-gray-600">Visits: {client.totalVisits}</span>
                             <button onClick={() => setViewClientHistory(client.name)} className="text-gold-600 font-bold text-xs flex items-center gap-1"><History size={14} /> History</button>
                          </div>
                       </div>
                    ))}
                 </div>
           ...
           <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden animate-fade-in pb-10 flex flex-col h-[calc(100vh-140px)]">
                       ...
                       {selectedIds.size > 0 && (
                          <div className="flex items-center gap-2 animate-fade-in">
                             ...
                             <div className="flex bg-white border border-gray-200 rounded-lg overflow-hidden shadow-sm">
                                <button onClick={() => openConfirmation(null, 'BulkInquiryMarkRead')} className="p-2 hover:bg-blue-50 text-blue-600 border-r border-gray-100" title="Mark Read"><Eye size={16}/></button>
                                <button onClick={() => openConfirmation(null, 'BulkDeleteInquiries')} className="p-2 hover:bg-red-50 text-red-500" title="Delete Selected"><Trash2 size={16}/></button>
                             </div>
                          </div>
                       )}
                       <div className="relative w-full md:w-96">
                          <Search className="absolute left-3 top-3 text-gray-400" size={18} />
                          <input type="text" placeholder="Search inquiries..." className="w-full pl-10 pr-4 py-2 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-gold-500" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} />
                       </div>
                    </div>
                    <div className="flex gap-2 w-full md:w-auto items-center">
                       <label className="text-xs font-bold text-gray-500 uppercase hidden md:inline">Status:</label>
                       <select value={activeInquiryStatus} onChange={(e) => setActiveInquiryStatus(e.target.value as InquiryStatus | 'All')} className="p-2 text-sm border border-gray-200 rounded focus:ring-1 focus:ring-gold-500 outline-none w-full md:w-auto">
              ...
              <div className="overflow-y-auto flex-1">
                {paginatedInquiries.length > 0 ? paginatedInquiries.map(inq => (
                  <div key={inq.id} className={`p-4 md:p-6 hover:bg-gray-50 transition-colors border-b border-gray-100 ${!inq.read ? 'bg-blue-50/30' : ''}`}>
                    <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-2 gap-2">
                      <div className="flex items-center gap-3 w-full">
                        <input type="checkbox" checked={selectedIds.has(inq.id)} onChange={() => handleSelectOne(inq.id)} className="w-4 h-4 rounded border-gray-300 text-gold-600 shrink-0" />
                        {!inq.read && <span className="w-2 h-2 bg-blue-500 rounded-full shrink-0"></span>}
                        <h3 className="font-bold text-gray-900 truncate">{inq.name}</h3>
                        <span className="text-gray-400 text-xs border border-gray-200 px-2 py-0.5 rounded whitespace-nowrap ml-auto md:ml-0">{inq.date}</span>
                      </div>
                      <div className="flex items-center gap-2 w-full md:w-auto justify-between md:justify-end">
                         <select value={inq.status || 'New'} onChange={(e) => onInquiryStatusChange(inq.id, e.target.value as InquiryStatus)} className={`text-xs font-bold border rounded px-2 py-1 outline-none cursor-pointer ${inq.status === 'Resolved' ? 'bg-green-100 text-green-700 border-green-200' : inq.status === 'Contacted' ? 'bg-blue-100 text-blue-700 border-blue-200' : inq.status === 'Follow-up Needed' ? 'bg-yellow-100 text-yellow-700 border-yellow-200' : 'bg-gray-100 text-gray-700 border-gray-200'}`}>
                            <option value="New">New</option>
                            <option value="Contacted">Contacted</option>
                            <option value="Follow-up Needed">Follow-up Needed</option>
                            <option value="Resolved">Resolved</option>
                         </select>
                         <button onClick={() => openConfirmation(inq.id, 'DeleteInquiry')} className="text-gray-400 hover:text-red-500 p-1"><Trash2 size={16} /></button>
                      </div>
                    </div>
                    <div className="text-sm text-gray-500 mb-2 flex flex-col md:flex-row md:gap-4 pl-7">
                      <span>{inq.email}</span>
                      <span>{inq.phone}</span>
                    </div>
                    <p className="text-gray-700 leading-relaxed bg-gray-50 p-3 rounded-lg border border-gray-100 ml-7 text-sm">"{inq.message}"</p>
                    <div className="mt-3 flex gap-2 ml-7">
                      <a href={`mailto:${inq.email}?subject=Re: Inquiry for Optimum Skin`} className="px-3 py-1.5 bg-black text-white text-xs font-bold rounded hover:bg-gold-600 transition-colors inline-block">Reply via Email</a>
                      {!inq.read && (<button onClick={() => onMarkInquiryRead(inq.id)} className="px-3 py-1.5 border border-gray-300 text-gray-600 text-xs font-bold rounded hover:bg-gray-100 transition-colors">Mark as Read</button>)}
                    </div>
                  </div>
                )) : (
                  <div className="p-8 text-center text-gray-500 h-full flex items-center justify-center">No inquiries found.</div>
                )}
              </div>

              {/* Pagination */}
              <div className="p-4 border-t border-gray-100 flex items-center justify-between bg-gray-50 flex-shrink-0">
                 <div className="text-xs text-gray-500">Page {currentInquiryPage} of {totalInquiryPages || 1}</div>
                 <div className="flex gap-2">
                    <button onClick={() => setCurrentInquiryPage(p => Math.max(1, p-1))} disabled={currentInquiryPage === 1} className="p-2 border rounded hover:bg-gray-100 disabled:opacity-50"><ChevronLeft size={16}/></button>
                    <button onClick={() => setCurrentInquiryPage(p => Math.min(totalInquiryPages, p+1))} disabled={currentInquiryPage >= totalInquiryPages} className="p-2 border rounded hover:bg-gray-100 disabled:opacity-50"><ChevronRight size={16}/></button>
                 </div>
              </div>
           </div>
        )}
        ...
        {activeTab === 'services' && (
           <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden animate-fade-in pb-10 flex flex-col h-[calc(100vh-140px)]">
              <div className="p-4 md:p-6 border-b border-gray-100 flex flex-col md:flex-row justify-between items-center gap-4 flex-shrink-0">
                 <div className="w-full md:w-auto">
                    <h2 className="text-lg font-bold text-gray-900">Services Management</h2>
                    <p className="text-sm text-gray-500 hidden md:block">Manage your clinic's service offerings.</p>
                 </div>
                 <div className="flex flex-col md:flex-row gap-3 w-full md:w-auto">
                    {selectedIds.size > 0 && (
                        <div className="flex items-center gap-2 animate-fade-in">
                           <div className="flex bg-white border border-gray-200 rounded-lg overflow-hidden shadow-sm">
                              <button onClick={() => openConfirmation(null, 'BulkEnableServices')} className="p-2 hover:bg-green-50 text-green-600 border-r border-gray-100 text-xs font-bold px-3">Enable</button>
                              <button onClick={() => openConfirmation(null, 'BulkDisableServices')} className="p-2 hover:bg-red-50 text-red-500 border-r border-gray-100 text-xs font-bold px-3">Disable</button>
                              <button onClick={() => openConfirmation(null, 'BulkDeleteServices')} className="p-2 hover:bg-gray-100 text-gray-600" title="Delete Selected"><Trash2 size={16}/></button>
                           </div>
                        </div>
                    )}
                    <div className="relative w-full md:w-64">
                       <Search className="absolute left-3 top-2.5 text-gray-400" size={16} />
                       <input type="text" placeholder="Search services..." className="w-full pl-9 pr-4 py-2 text-sm border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-gold-500" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} />
                    </div>
                    <button onClick={openAddServiceModal} className="bg-black text-white px-4 py-2 rounded-lg text-sm font-bold flex items-center justify-center gap-2 hover:bg-gold-600 transition-colors w-full md:w-auto"><Plus size={16} /> Add Service</button>
                 </div>
              </div>
              ...
              <div className="overflow-y-auto flex-1 p-4 md:p-0">
                 ...
                 <div className="hidden md:block">
                   <table className="w-full text-left border-collapse min-w-[800px]">
                      <thead>
                         <tr className="bg-gray-50 text-gray-500 text-xs uppercase tracking-wider sticky top-0 z-10 shadow-sm">
                            <th className="p-4 w-10">
                               <input type="checkbox" onChange={handleSelectAll} checked={paginatedServices.length > 0 && paginatedServices.every(s => selectedIds.has(s.id))} className="w-4 h-4 rounded border-gray-300 text-gold-600 focus:ring-gold-500" />
                            </th>
                      ...
                      <tbody className="divide-y divide-gray-100 text-sm">
                         {paginatedServices.map((service) => (
                            <tr key={service.id} className={`hover:bg-gray-50 transition-colors ${selectedIds.has(service.id) ? 'bg-gold-50/50' : ''}`}>
                               <td className="p-4"><input type="checkbox" checked={selectedIds.has(service.id)} onChange={() => handleSelectOne(service.id)} className="w-4 h-4 rounded border-gray-300 text-gold-600 focus:ring-gold-500" /></td>
                               <td className="p-4">
                                  <div className="flex items-center gap-3">
                               ...
                               <td className="p-4 text-center">
                                  <div className="flex justify-center">
                                     <button onClick={() => onToggleServiceAvailability(service.id, !service.available)} className={`relative w-10 h-5 rounded-full transition-colors ${service.available ? 'bg-green-500' : 'bg-gray-300'}`}>
                                        <div className={`absolute top-1 left-1 bg-white w-3 h-3 rounded-full transition-transform ${service.available ? 'translate-x-5' : 'translate-x-0'}`}></div>
                                     </button>
                                  </div>
                               </td>
                               <td className="p-4 text-right">
                                  <div className="flex items-center justify-end gap-2">
                                     <button onClick={() => openEditServiceModal(service)} className="p-1.5 text-blue-600 hover:bg-blue-50 rounded" title="Edit Service"><Edit size={18} /></button>
                                     <button onClick={() => openConfirmation(service.id, 'DeleteService')} className="p-1.5 text-red-600 hover:bg-red-50 rounded" title="Delete Service"><Trash2 size={18} /></button>
                                  </div>
                               </td>
                            </tr>
                         ))}
                 ...
                 <div className="md:hidden space-y-4">
                    {paginatedServices.map((service) => (
                       <div key={service.id} className={`bg-white p-4 rounded-xl border shadow-sm ${selectedIds.has(service.id) ? 'border-gold-400 bg-gold-50' : 'border-gray-100'}`}>
                          <div className="flex items-center gap-3 mb-3">
                             <input type="checkbox" checked={selectedIds.has(service.id)} onChange={() => handleSelectOne(service.id)} className="w-5 h-5 rounded border-gray-300 text-gold-600" />
                             <img src={service.image} alt={service.title} className="w-12 h-12 rounded object-cover" />
                             <div className="flex-1 min-w-0">
                                <h3 className={`font-bold text-gray-900 truncate ${!service.available && 'text-gray-400 line-through'}`}>{service.title}</h3>
                          ...
                          <div className="flex justify-between items-center pt-3 border-t border-gray-100">
                             <div className="flex items-center gap-2">
                                ...
                                <button onClick={() => onToggleServiceAvailability(service.id, !service.available)} className={`relative w-8 h-4 rounded-full transition-colors ${service.available ? 'bg-green-500' : 'bg-gray-300'}`}>
                                   <div className={`absolute top-0.5 left-0.5 bg-white w-3 h-3 rounded-full transition-transform ${service.available ? 'translate-x-4' : 'translate-x-0'}`}></div>
                                </button>
                             </div>
                             <div className="flex gap-2">
                                <button onClick={() => openEditServiceModal(service)} className="p-1.5 bg-blue-50 text-blue-600 rounded"><Edit size={16}/></button>
                                <button onClick={() => openConfirmation(service.id, 'DeleteService')} className="p-1.5 bg-red-50 text-red-600 rounded"><Trash2 size={16}/></button>
                             </div>
                          </div>
                       </div>
                    ))}
              ...
              {/* Pagination */}
              <div className="p-4 border-t border-gray-100 flex items-center justify-between bg-gray-50 flex-shrink-0">
                 <div className="text-xs text-gray-500">Page {currentServicePage} of {totalServicePages || 1}</div>
                 <div className="flex gap-2">
                    <button onClick={() => setCurrentServicePage(p => Math.max(1, p-1))} disabled={currentServicePage === 1} className="p-2 border rounded hover:bg-gray-100 disabled:opacity-50"><ChevronLeft size={16}/></button>
                    <button onClick={() => setCurrentServicePage(p => Math.min(totalServicePages, p+1))} disabled={currentServicePage >= totalServicePages} className="p-2 border rounded hover:bg-gray-100 disabled:opacity-50"><ChevronRight size={16}/></button>
                 </div>
              </div>
           </div>
        )}
                    ...
                    <div className="p-6 space-y-6">
                       <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                          <div className="p-4 border border-gray-200 rounded-xl bg-white hover:border-gold-300 transition-colors">
                             ...
                             <button onClick={onDownloadBackup} className="w-full py-2 bg-black text-white rounded-lg text-sm font-bold hover:bg-gold-600 transition-colors">Download Backup</button>
                          </div>
                          <div className="p-4 border border-gray-200 rounded-xl bg-white hover:border-gold-300 transition-colors relative">
                             ...
                             <input type="file" accept=".json" onChange={(e) => e.target.files && onUploadBackup(e.target.files[0])} className="absolute inset-0 w-full h-full opacity-0 cursor-pointer" />
                             <button className="w-full py-2 border border-gray-300 text-gray-700 rounded-lg text-sm font-bold hover:bg-gray-100 transition-colors">Select File to Upload</button>
                          </div>
                       </div>
                       ...
                       <div className="mt-8 pt-6 border-t border-gray-200">
                          ...
                          <div className="flex flex-col md:flex-row items-center justify-between p-4 bg-red-50 border border-red-100 rounded-lg gap-4">
                             ...
                             <button onClick={() => openConfirmation(null, 'ResetDatabase')} className="px-4 py-2 bg-white border border-red-200 text-red-600 rounded-lg text-sm font-bold hover:bg-red-600 hover:text-white transition-colors whitespace-nowrap">Reset Database</button>
                          </div>
                       </div>
                    </div>
                 </div>
      ...
      {isServiceModalOpen && (
         <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-fade-in">
            <div className="bg-white rounded-xl shadow-2xl max-w-lg w-full p-6 animate-slide-in max-h-[90vh] overflow-y-auto">
               <h2 className="text-xl font-serif font-bold text-gray-900 mb-4">{editingService ? 'Edit Service' : 'Add New Service'}</h2>
               <form onSubmit={handleServiceSubmit} className="space-y-4">
                  <div><label className="block text-sm font-medium text-gray-700 mb-1">Service Title</label><input type="text" required value={serviceFormData.title} onChange={(e) => setServiceFormData({...serviceFormData, title: e.target.value})} className="w-full p-2 border border-gray-300 rounded focus:ring-2 focus:ring-gold-500 outline-none" /></div>
                  <div><label className="block text-sm font-medium text-gray-700 mb-1">Category</label><select value={serviceFormData.category} onChange={(e) => setServiceFormData({...serviceFormData, category: e.target.value as ServiceCategory})} className="w-full p-2 border border-gray-300 rounded focus:ring-2 focus:ring-gold-500 outline-none">{Object.values(ServiceCategory).map(cat => (<option key={cat} value={cat}>{cat}</option>))}</select></div>
                  <div><label className="block text-sm font-medium text-gray-700 mb-1">Price Range</label><input type="text" placeholder="e.g., ₱3,500 - ₱5,000" required value={serviceFormData.priceRange} onChange={(e) => setServiceFormData({...serviceFormData, priceRange: e.target.value})} className="w-full p-2 border border-gray-300 rounded focus:ring-2 focus:ring-gold-500 outline-none" /></div>
                  <div><label className="block text-sm font-medium text-gray-700 mb-1">Description</label><textarea rows={3} required value={serviceFormData.description} onChange={(e) => setServiceFormData({...serviceFormData, description: e.target.value})} className="w-full p-2 border border-gray-300 rounded focus:ring-2 focus:ring-gold-500 outline-none" /></div>
                  <div><label className="block text-sm font-medium text-gray-700 mb-1">Image URL</label><input type="url" placeholder="https://..." required value={serviceFormData.image} onChange={(e) => setServiceFormData({...serviceFormData, image: e.target.value})} className="w-full p-2 border border-gray-300 rounded focus:ring-2 focus:ring-gold-500 outline-none" /></div>
                  <div className="flex justify-end gap-3 mt-6"><button type="button" onClick={() => setIsServiceModalOpen(false)} className="px-4 py-2 border border-gray-300 rounded text-gray-700 hover:bg-gray-50">Cancel</button><button type="submit" className="px-4 py-2 bg-black text-white rounded hover:bg-gold-600 transition-colors">{editingService ? 'Update Service' : 'Add Service'}</button></div>
               </form>
            </div>
         </div>
      )}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-fade-in">
          <div className="bg-white rounded-xl shadow-2xl max-w-lg w-full flex flex-col animate-slide-in overflow-hidden max-h-[80vh]">
             <div className="bg-gold-50 p-6 border-b border-gold-100 flex justify-between items-start">
                <div>
                   <h2 className="text-xl font-serif font-bold text-gray-900">{selectedCalendarDate.toLocaleDateString('default', { weekday: 'long', month: 'long', day: 'numeric' })}</h2>
                   <p className="text-gold-600 text-sm mt-1">{selectedDayAppointments.length} Appointments Scheduled</p>
                </div>
                <button onClick={() => setSelectedCalendarDate(null)} className="p-2 hover:bg-gold-100 rounded-full text-gold-600"><X size={24} /></button>
             </div>
             <div className="p-6 overflow-y-auto flex-1">
                {selectedDayAppointments.length > 0 ? (
                         ...
                         <div key={apt.id} className="relative pl-4 border-l-2 border-gold-200 pb-4 last:pb-0">
                            ...
                            <div className="bg-white p-4 rounded-lg border border-gray-100 shadow-sm hover:shadow-md transition-shadow cursor-pointer" onClick={() => setViewAppointment(apt)}>
                               <div className="flex justify-between items-start mb-2">
                                  ...
                                  <span className={`px-2 py-0.5 rounded text-[10px] uppercase font-bold border ${getStatusBadge(apt.status)}`}>{apt.status}</span>
                               </div>
                   ...
                   <div className="text-center py-12 text-gray-400"><CalendarIcon size={48} className="mx-auto mb-4 opacity-20" /><p>No appointments scheduled for this day.</p></div>
                )}
             </div>
             <div className="p-4 bg-gray-50 border-t border-gray-100 text-right">
                <button onClick={() => setSelectedCalendarDate(null)} className="px-6 py-2 bg-gray-900 text-white font-bold rounded-lg hover:bg-gray-800 transition-colors">Close</button>
             </div>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm animate-fade-in">
          <div className="bg-white rounded-xl shadow-2xl max-w-sm w-full p-6 animate-slide-in">
             <div className="flex flex-col items-center text-center">
                <div className={`w-12 h-12 rounded-full flex items-center justify-center mb-4 ${confirmationAction.type?.includes('Confirm') ? 'bg-green-100 text-green-600' : confirmationAction.type?.includes('Cancel') ? 'bg-red-100 text-red-600' : confirmationAction.type?.includes('Trash') || confirmationAction.type === 'DeleteService' || confirmationAction.type?.includes('DeleteInquiry') ? 'bg-gray-100 text-gray-600' : confirmationAction.type === 'Restore' ? 'bg-gold-100 text-gold-600' : confirmationAction.type === 'ResetDatabase' ? 'bg-red-100 text-red-600' : 'bg-blue-100 text-blue-600'}`}>
                ...
                <h3 className="text-lg font-bold text-gray-900 mb-2">
                  ...
                  {confirmationAction.type === 'BulkEnableServices' && 'Enable Services'}
                  {confirmationAction.type === 'BulkDisableServices' && 'Disable Services'}
                  {confirmationAction.type === 'BulkDeleteServices' && 'Delete Services'}
                  {confirmationAction.type === 'DeleteInquiry' && 'Delete Inquiry'}
                  {confirmationAction.type === 'BulkDeleteInquiries' && 'Delete Inquiries'}
                  {confirmationAction.type === 'BulkInquiryMarkRead' && 'Mark Inquiries Read'}
                </h3>
                <p className="text-gray-500 text-sm mb-6">
                  {confirmationAction.type === 'ResetDatabase'
                    ...
                    : confirmationAction.type?.includes('DeleteInquiry')
                    ? 'This will permanently remove these inquiries.'
                ...
                <div className="flex gap-3 w-full">
                   <button onClick={closeConfirmation} className="flex-1 py-2.5 border border-gray-300 rounded-lg text-gray-700 font-medium hover:bg-gray-50 transition-colors">No, Go Back</button>
                   <button onClick={handleConfirmAction} className={`flex-1 py-2.5 rounded-lg text-white font-medium shadow-md transition-colors ${confirmationAction.type?.includes('Confirm') ? 'bg-green-600 hover:bg-green-700' : confirmationAction.type?.includes('Cancel') ? 'bg-red-600 hover:bg-red-700' : confirmationAction.type?.includes('Trash') || confirmationAction.type === 'DeleteService' || confirmationAction.type?.includes('DeleteInquiry') ? 'bg-gray-600 hover:bg-gray-700' : confirmationAction.type === 'Restore' ? 'bg-gold-600 hover:bg-gold-700' : confirmationAction.type === 'ResetDatabase' ? 'bg-red-600 hover:bg-red-700' : 'bg-blue-600 hover:bg-blue-700'}`}>{confirmationAction.type === 'ResetDatabase' ? 'Yes, Delete All' : 'Yes, Confirm'}</button>
                </div>
             </div>
          </div>
        </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-fade-in">
          <div className="bg-white rounded-xl shadow-2xl max-w-2xl w-full max-h-[85vh] flex flex-col animate-slide-in">
             <div className="p-6 border-b border-gray-100 flex justify-between items-start">
                <div><h2 className="text-xl md:text-2xl font-serif font-bold text-gray-900 flex items-center gap-2"><History className="text-gold-500" size={24} /> Client History</h2><p className="text-gray-500 mt-1 text-sm md:text-base">Records for <span className="font-bold text-gray-800">{viewClientHistory}</span></p></div>
                <button onClick={() => setViewClientHistory(null)} className="p-2 hover:bg-gray-100 rounded-full text-gray-500"><XCircle size={24} /></button>
             </div>
             <div className="p-6 bg-gray-50 border-b border-gray-100">
                <div className="flex flex-col md:flex-row gap-4 md:gap-8 mb-6">
                  <div className="flex items-center gap-2 text-sm text-gray-600"><Phone size={16} className="text-gold-600" /> <span>{clientInfo.phone || 'N/A'}</span></div>
                  <div className="flex items-center gap-2 text-sm text-gray-600"><Mail size={16} className="text-gold-600" /> <span>{clientInfo.email || 'N/A'}</span></div>
                </div>
                {clientHistoryStats && (
                   <div className="grid grid-cols-3 gap-4 mb-2">
                      <div className="bg-blue-50 p-3 rounded-lg border border-blue-100 text-center"><p className="text-xs text-blue-500 font-bold uppercase">Completed</p><p className="text-xl font-bold text-blue-700">{clientHistoryStats.completed}</p></div>
                      <div className="bg-red-50 p-3 rounded-lg border border-red-100 text-center"><p className="text-xs text-red-500 font-bold uppercase">Cancelled</p><p className="text-xl font-bold text-red-700">{clientHistoryStats.cancelled}</p></div>
                      <div className="bg-gold-50 p-3 rounded-lg border border-gold-100 text-center"><p className="text-xs text-gold-600 font-bold uppercase">Top Service</p><p className="text-sm font-bold text-gold-800 truncate">{clientHistoryStats.favoriteService}</p></div>
                   </div>
                )}
             </div>
                ...
                <div className="space-y-4">
                   {clientHistoryAppointments.map((apt, index) => (
                      <div key={apt.id} className="relative pl-6 border-l-2 border-gray-200 pb-2 last:pb-0">
                         <div className={`absolute -left-[9px] top-0 w-4 h-4 rounded-full border-2 border-white ${index === 0 ? 'bg-gold-500' : 'bg-gray-300'}`}></div>
                         <div className="bg-white p-4 rounded-lg border border-gray-100 shadow-sm hover:shadow-md transition-shadow">
                            <div className="flex justify-between items-start mb-2"><h4 className="font-bold text-gray-900">{apt.service}</h4><span className={`px-2 py-0.5 rounded text-[10px] uppercase font-bold border ${getStatusBadge(apt.status)}`}>{apt.status}</span></div>
                            <div className="text-sm text-gray-500 flex items-center gap-2 mb-2"><CalendarIcon size={14} /> {apt.date}<span className="text-gray-300">|</span><Clock size={14} /> {apt.time}</div>
                            {apt.notes && (<div className="text-xs text-gray-600 bg-gray-50 p-2 rounded italic">"{apt.notes}"</div>)}
                         </div>
                      </div>
                   ))}
                   {clientHistoryAppointments.length === 0 && (<div className="text-center text-gray-500 py-8">No history found for this client.</div>)}
                </div>
             </div>
             <div className="p-4 border-t border-gray-100 bg-gray-50 rounded-b-xl text-right">
                <button onClick={() => setViewClientHistory(null)} className="px-6 py-2 bg-gray-900 text-white font-bold rounded-lg hover:bg-gray-800 transition-colors">Close</button>
             </div>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-fade-in">
          <div className="bg-white rounded-xl shadow-2xl max-w-lg w-full flex flex-col animate-slide-in overflow-hidden max-h-[90vh] overflow-y-auto">
             ...
             <div className="p-4 bg-gray-50 border-t border-gray-100 flex flex-wrap justify-end gap-3">
                 {viewAppointment.status === 'Pending' && (<><button onClick={() => openConfirmation(viewAppointment.id, 'Cancel')} className="px-4 py-2 border border-red-200 text-red-600 bg-white hover:bg-red-50 rounded-lg text-sm font-bold transition-colors">Reject</button><button onClick={() => openConfirmation(viewAppointment.id, 'Confirm')} className="px-4 py-2 bg-green-600 text-white hover:bg-green-700 rounded-lg text-sm font-bold transition-colors shadow-sm">Approve</button></>)}
                 {viewAppointment.status === 'Confirmed' && (<><button onClick={() => openConfirmation(viewAppointment.id, 'Cancel')} className="px-4 py-2 border border-red-200 text-red-600 bg-white hover:bg-red-50 rounded-lg text-sm font-bold transition-colors">Cancel</button><button onClick={() => openConfirmation(viewAppointment.id, 'Trash')} className="px-4 py-2 border border-gray-300 text-gray-500 bg-white hover:bg-gray-100 rounded-lg text-sm font-bold transition-colors">Trash</button><button onClick={() => openConfirmation(viewAppointment.id, 'Complete')} className="px-4 py-2 bg-blue-600 text-white hover:bg-blue-700 rounded-lg text-sm font-bold transition-colors shadow-sm">Complete</button></>)}
                 {(viewAppointment.status === 'Completed' || viewAppointment.status === 'Cancelled') && (<button onClick={() => openConfirmation(viewAppointment.id, 'Trash')} className="px-4 py-2 border border-gray-300 text-gray-500 bg-white hover:bg-gray-100 rounded-lg text-sm font-bold transition-colors">Trash</button>)}
                 {viewAppointment.status === 'Trash' && (<button onClick={() => openConfirmation(viewAppointment.id, 'Restore')} className="px-4 py-2 bg-gold-600 text-white hover:bg-gold-700 rounded-lg text-sm font-bold transition-colors shadow-sm">Restore</button>)}
      ...
      {tooltipData && (
        <div className="fixed z-50 bg-gray-900 text-white p-3 rounded-lg shadow-xl text-xs pointer-events-none max-w-xs animate-fade-in hidden md:block" style={{ top: tooltipData.rect.top - 8, left: tooltipData.rect.left, transform: 'translateY(-100%)' }}>
          ...
          <div className={`inline-block px-1.5 py-0.5 rounded text-[10px] uppercase font-bold border ${getStatusBadge(tooltipData.appointment.status)}`}>{tooltipData.appointment.status}</div>
          {tooltipData.appointment.notes && (<div className="mt-2 pt-2 border-t border-gray-700 italic text-gold-100 bg-gray-800 p-2 rounded">"{tooltipData.appointment.notes}"</div>)}
